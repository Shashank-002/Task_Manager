...
<body class="bg-gray-100">
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-md mt-12">
        ...
        <form action="{{ route('tasks.update', $taskIndex) }}" method="POST" novalidate>
            @csrf
            @method('PUT')

            <!-- Task input with error styling -->
            <div class="mb-4">
                <input type="text" name="task" value="{{ old('task', $task) }}" class="w-full p-2 border rounded @error('task') border-red-500 @enderror" required>
